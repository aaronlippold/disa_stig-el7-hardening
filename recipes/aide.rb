...
aide_config = {}.merge(node.default['stig']['aide'].to_h)
if %w[rhel fedora centos].include?(node['platform'])
  platform_version = node['platform_version'].to_i.to_s

  # Set the rules by merging node.default['stig']['aide']['rules'] with
  # a platform version specific aide_config['rules_rhel']['6'] or
  # node.default['stig']['aide']['rules_rhel']['default'], which is for RHEL 7+
  aide_config['rules'] =
  ...
  # Set the paths by merging node.default['stig']['aide']['paths'] with
  # a platform version specific aide_config['paths_rhel']['6'] or
  # node.default['stig']['aide']['paths_rhel']['default'], which is for RHEL 7+
  aide_config['paths'] =
...
template node.default['stig']['aide']['config_file'] do
  action :create
  group 'root'
  mode 0o600
  owner 'root'
...
aide_database = node.default['stig']['aide']['database'].gsub('@@{DBDIR}', node.default['stig']['aide']['dbdir'])
